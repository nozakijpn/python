...
tyouhuku_th = 10
anchor_th = 0.05
test_anchor_num = 2#評価したいアンカーのクラスタ番号の指定
ques_anchor_cluster_num = 2#評価したいアンカーのクラスタ番号
...
def main(speaker_name,ivpath):
    ...
    for i in range(roop_num):
        ...
        aveiv = []
        for item in lis:
            iv = ivmodule.get_ivdata(ivpath,item)
            aveiv.append(iv)
        
        ave = np.array(aveiv)
        ave1 = ave.astype(np.float)
        ave2 = np.mean(ave1,axis=0)
        
        lis2 = ivmodule.get_thlist_for_averagefile(ivpath,ave2,filelist)
